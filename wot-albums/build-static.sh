#!/bin/bash
# Build single static HTML file for WoT Albums
# Concatenates CSS + JS inline into one HTML file

DIR="$(cd "$(dirname "$0")" && pwd)"
OUT="$DIR/index-static.html"
    ...
    <div class="lyrics-panel" id="lyrics-panel">
      ...
      <div class="sync-metrics" id="sync-metrics">
        <span class="sync-metric">
          ...
          <span class="sync-metric-value" id="sync-pcetw">—</span>% in sync
        </span>
        <span class="sync-metric-sep">|</span>
        <span class="sync-metric">
          Avg drift: <span class="sync-metric-value" id="sync-aae">—</span>ms
        </span>
        <span class="sync-metric-sep">|</span>
        <span class="sync-metric">
          <span class="sync-metric-dot" style="background:var(--sync-misaligned)"></span>
          <span class="sync-metric-value" id="sync-misaligned">—</span> misaligned
        </span>
        <span class="sync-metric-sep">|</span>
    ...
    <div class="player-content">
      <div class="player-track-info">
        ...
        <div>
          ...
          <div class="player-album-name" id="player-album-name">—</div>
        </div>
      </div>
...
cat "$DIR/js/albums.js" >> "$OUT"
cat "$DIR/js/lyrics-timings.js" >> "$OUT"
cat "$DIR/js/sync-reference.js" >> "$OUT"
cat "$DIR/js/sync-analysis.js" >> "$OUT"
cat "$DIR/js/app.js" >> "$OUT"
